...
def render_markdown(snapshot):
    ...
    ln(f"**Timestamp:** {ts}")
    ln(f"**Script:** cluster_snapshot.py v2.0 (CSW POV Template)")
    br()
    ...
    # ── Footer ───────────────────────────────────────────────
    h(2, "Notes")
    ln("- Snapshot generated by `cluster_snapshot.py` (CSW POV Template, v2.0)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="CSW Cluster Snapshot Tool")
